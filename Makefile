...
build: init
	# build $(app)
	go build -ldflags "-X github.com/LumiWave/baseapp/base.AppVersion=$(revVersion).$(buildTime)" -o $(output)/$(app) main.go
...
test: 
	...
	go test github.com/LumiWave/inno-dashboard -v -timeout 30s \
